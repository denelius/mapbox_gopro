...
var map = L.mapbox.map('map', 'newfields.a87f6313').setView([33.769, -84.351], 15);
...
function nearest_point(lat, lng) {
	...
	var it;
	for (var i = 0, e = pjson.length; i < e; ++i) {
		var ld = lineDistance(lng.toString(), lat.toString(), pjson[i].x, pjson[i].y);
		if (ld < dist) {
			dist = ld
			it = pjson[i];
		}
